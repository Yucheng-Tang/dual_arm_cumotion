...
class MotionGen(MotionGenConfig):
    ...
    def plan_single(
        ...
        # print("plan_single, link_poses", link_poses)
    ...
    def check_start_state(
        self, start_state: JointState, link_poses: Optional[Dict[str, Pose]] = None
        ...
        # print("DEBUG 1:", joint_position)
        metrics = self.rollout_fn.rollout_constraint(
            joint_position,
            use_batch_env=False,
        )
        # print("DEBUG 2:", joint_position)
    ...
    def _plan_attempts(
        ...
        import copy
        link_poses_copy = copy.deepcopy(link_poses)
        start_time = time.time()
        valid_query = True
        plan_config = plan_config.clone()
        if plan_config.check_start_validity:
            valid_query, status = self.check_start_state(start_state, link_poses)
            if not valid_query:
                result = MotionGenResult(
                    success=torch.as_tensor([False], device=self.tensor_args.device),
                    valid_query=valid_query,
    ...
    def _plan_from_solve_state(
        ...
        # print(goal_pose, link_poses, self.compute_kinematics(ik_result.js_solution).ee_pose, self.compute_kinematics(ik_result.js_solution).link_pose)
        ...
        if plan_config.enable_opt:
            with profiler.record_function("motion_gen/setup_trajopt_seeds"):
                ...
                if (
                    ...
                    # print("goal_state:", goal_state, self.compute_kinematics(goal_state).ee_pose, self.compute_kinematics(goal_state).link_pose)
            ...
            # print("DEBUG: trajopt_seed_traj", trajopt_seed_traj)
